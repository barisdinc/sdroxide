...
impl Device {
    ...
    /// Query the argument info description for the device's own settings.
    ///
    /// **Added in this vendored copy; not present upstream.** See
    /// `vendor/soapysdr/PROVENANCE.md`. `writeSetting`/`readSetting` are wrapped
    /// upstream but the call that says *which* settings a driver has — and of
    /// what type, with what options and defaults — is not, and the raw device
    /// pointer it needs is private to this module. Without it a host can set a
    /// driver setting it already knows the name of, but cannot discover one.
    ///
    /// This is the exact shape of [`Self::stream_args_info`] one field over,
    /// which is why it is safe to carry as a patch: it adds no new unsafety and
    /// no new FFI helper, only a second caller of `arg_info_result`.
    pub fn setting_info(&self) -> Result<Vec<ArgInfo>, Error> {
        unsafe {
            arg_info_result(|len_ptr| {
                SoapySDRDevice_getSettingInfo(self.inner.ptr, len_ptr)
            })
        }
    }
