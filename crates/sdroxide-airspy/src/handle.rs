...
//!
//! Same shape as `sdroxide-airspyhf` and [`sdroxide_rtlsdr`]: one blocking
//! thread owns the receiver, control goes in over a crossbeam channel, samples
//! come back out through an `rtrb` ring of interleaved `f32`.
...
impl RxStats {
    ...
    pub(crate) fn tick(&mut self, trace: &Trace) {
        ...
        if self.win_dropped > 0 || self.win_errors > 0 || silent {
            ...
            tracing::debug!(
                "Airspy RX: {} samples ({ksps:.1} ksps) over {:.2}s; total {}; {}{}",
                self.win_samples,
                dt.as_secs_f64(),
                self.total_samples,
                self.clock_error(),
...
/// An open Airspy R2 / Mini.
pub struct AirspyHandle {
    rx: Consumer<f32>,
    ctrl: Sender<Ctrl>,
    shared: Arc<Shared>,
    opened_at: Instant,
...
impl AirspyHandle {
    ...
    /// succeeded or failed.
    pub fn open(cfg: &AirspyConfig, center_hz: f64) -> Result<AirspyHandle> {
...
///
/// Half a second is affordable here in a way it is not on the HackRF: the top
/// complex rate is 10 Msps, so this is 40 MB at worst — and unlike that radio
/// the rate cannot be raised beyond what the list offers, so the figure is
/// bounded by the hardware rather than by a config field.
...
mod tests {
    ...
    fn the_ring_holds_at_least_half_a_second_and_an_even_number_of_floats() {
        for rate in [2.5e6, 3.0e6, 6.0e6, 10.0e6] {
