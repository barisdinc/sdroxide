                    ...
                    Some(msg) => match msg {
                        ...
                        ServerMsg::RxAudio { audio_b64 } => {
                            ...
                            let Some(payload) = attempt.payload.clone() else {
                                ...
                                // Reported only when the best attempt needed
                                // one — the common (unshifted) case says
                                // nothing extra.
                                let frac = if attempt.frac_hz == 0.0 {
                                    String::new()
                                } else {
                                    format!(", {:+.1} Hz pre-shift", attempt.frac_hz)
                                };
                                self.log(match (attempt.synced, attempt.header_shift) {
                                    (false, _) => format!(
                                        "demod: no timing sync ({} samples, score {:.2} < {SYNC_THRESHOLD}{frac})",
                                        attempt.input_samples, attempt.sync_score
                                    ),
                                    (true, None) => format!(
                                        "demod: synced (score {:.2}{frac}) but no shift in {MAX_SHIFT_DOWN}..+{MAX_SHIFT_UP} bins found a plausible header",
                                        attempt.sync_score
                                    ),
                                    (true, Some(shift)) => format!(
                                        "demod: synced (score {:.2}{frac}), header at shift {shift:+} ({} bytes claimed) but no shift's data passed CRC",
                                        attempt.sync_score,
